...
pokemon_info = []
for dresssphere in dressphere_randomize.dresspheres:
    pokemon_info.append(dresssphere.return_as_list())
...
class Some_Widgets(GUI):  # inherits from the GUI class
    def __init__(self, parent, controller):
        ...
        frame1 = tk.LabelFrame(self, frame_styles, text="Dress Selection")
        frame1.place(rely=0.05, relx=0.02, height=400, width=250)

        self.frame2 = tk.LabelFrame(self, frame_styles, text="Some widgets")
        self.frame2.place(rely=0.05, relx=0.3, height=500, width=666)


        # button1 = tk.Button(self.frame2, text="tk button", command=lambda: initialize())
        #
        # button1.pack()
        # button2 = ttk.Button(self.frame2, text="ttk button", command=lambda: Refresh_data())
        # button2.pack()

        # Var1 = tk.IntVar()
        # Var2 = tk.IntVar()
        # Cbutton1 = tk.Checkbutton(self.frame2, text="tk CheckButton1", variable=Var1, onvalue=1, offvalue=0)
        # Cbutton1.pack()
        # Cbutton2 = tk.Checkbutton(self.frame2, text="tk CheckButton2", variable=Var2, onvalue=1, offvalue=0)
        # Cbutton2.pack()
        #
        # Cbutton3 = ttk.Checkbutton(self.frame2, text="ttk CheckButton1", variable=Var1, onvalue=1, offvalue=0)
        # Cbutton3.pack()
        # Cbutton3 = ttk.Checkbutton(self.frame2, text="ttk CheckButton2", variable=Var2, onvalue=1, offvalue=0)
        # Cbutton3.pack()

        # Lbox1 = tk.Listbox(self.frame2, selectmode="multiple")
        # Lbox1.insert(1, "This is a tk ListBox")
        # Lbox1.insert(2, "Github")
        # Lbox1.insert(3, "Python")
        # Lbox1.insert(3, "StackOverflow")
        # Lbox1.pack(side="top",anchor="nw")

        # Var3 = tk.IntVar()
        # R1 = tk.Radiobutton(self.frame2, text="tk Radiobutton1", variable=Var3, value=1)
        # R1.pack()
        # R2 = tk.Radiobutton(self.frame2, text="tk Radiobutton2", variable=Var3, value=2)
        # R2.pack()
        # R3 = tk.Radiobutton(self.frame2, text="tk Radiobutton3", variable=Var3, value=3)
        # R3.pack()
        #
        # R4 = tk.Radiobutton(self.frame2, text="ttk Radiobutton1", variable=Var3, value=1)
        # R4.pack()
        # R5 = tk.Radiobutton(self.frame2, text="ttk Radiobutton2", variable=Var3, value=2)
        # R5.pack()
        # R6 = tk.Radiobutton(self.frame2, text="ttk Radiobutton3", variable=Var3, value=3)
        # R6.pack()

        # This is a treeview.
        self.tree = ttk.Treeview(frame1)
        column_list_account = ["Dress ID", "Dress Name"]
        self.tree['columns'] = column_list_account
        self.tree["show"] = "headings"  # removes empty column
        self.tree.heading(column_list_account[0], text=column_list_account[0])
        self.tree.column(column_list_account[0], width=50)
        self.tree.heading(column_list_account[1], text=column_list_account[1])
        self.tree.column(column_list_account[1], width=200)
        self.tree.place(relheight=1, relwidth=0.995)
        ...
        def Load_data():
            for row in pokemon_info:
                self.tree.insert("", "end", values=row,text=row[1])

        def Refresh_data():
            # Deletes the data in the current treeview and reinserts it.
            self.tree.delete(*self.tree.get_children())  # *=splat operator
            Load_data()
    ...
    def OnClick(self, event):
        ...
        self.refresh_frame2()
        for dress in dressphere_randomize.dresspheres:
            if dress.dress_name == self.tree.item(item, "text"):
                dress_title = tk.Text(self.frame2, height='1', font='Helvetica 18 bold')
                dress_title.insert("insert", self.tree.item(item, "text").capitalize())
                dress_title.pack(side='top', anchor='nw')

                dress_sub_title = tk.Text(self.frame2, height='1', font='Helvetica 13')
                dress_sub_title.insert("insert", "Click on an ability on the left to change, then choose an ability on the right to replace it.")
                dress_sub_title.pack(side='top', anchor='nw')

                column_list_account = ["Ability", "Required"]
                self.ability_tree = ttk.Treeview(self.frame2)
                self.ability_tree['columns'] = column_list_account
                self.ability_tree["show"] = "headings"  # removes empty column
                self.ability_tree.heading(column_list_account[0], text=column_list_account[0])
                self.ability_tree.column(column_list_account[0], width=125)
                self.ability_tree.heading(column_list_account[1], text=column_list_account[1])
                self.ability_tree.column(column_list_account[1], width=125)
                self.ability_tree.pack(side='left', anchor='n', fill='y')
                treescroll = tk.Scrollbar(self.frame2)
                treescroll.configure(command=self.ability_tree.yview)
                self.ability_tree.configure(yscrollcommand=treescroll.set)
                treescroll.pack(side="left",fill="y")

                optVariable = tk.StringVar(self.frame2)
                optVariable.set("  Select   ")
                all_ability_names = []
                for ability in dressphere_randomize.global_abilities:
                ...
                for ability_tuple in dress.abilities:
                    current = [dressphere_randomize.translate_ability(ability_tuple[1]), dressphere_randomize.translate_ability(ability_tuple[0])]
                    self.ability_tree.insert("", "end", values=current,text=current[0])
                    #self.ability_tree.bind("<Button-1>", self.OnClick)

        # button1 = tk.Button(self.frame2, text="tk button", command=lambda: initialize())
        # button1.pack()
        #Onclick, show the stuff according to "self.tree.item" to show stuff in other frame
